...
# Load the local sentence-transformer model instance (cached singleton)
model = get_embedding_model()
COLLECTION_NAME = "financial_news"

def finalizer_subagent(state: AgentState) -> AgentState:
    print(f"\n[SUBAGENT] Finalizer: Saving state...")
    ...
    try:
        # Save core article fields (Wrapped in text())
        db.execute(
            text("""
        ...
        db.commit()
        print("--> Saved to Neon Database successfully.")
        
    except Exception as e:
        db.rollback()
        print(f"Postgres Storage Error: {e}")
    ...
    if not state["is_duplicate"]:
        try:
            ...
            print("--> Indexed in Qdrant Cloud successfully.")
